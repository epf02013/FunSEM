...
source(here("latent_growth_curve_model.r"))
df = data.frame(
...
ec_cm_configural <- define_latent_growth_curve_model(df, "CONFIGURAL")

ec_cm_weak <- define_latent_growth_curve_model(df,"WEAK")

ec_cm_partial_strong <- define_latent_growth_curve_model(
 df,
 "PARTIAL_STRONG",
 c(2,3)
)

ec_cm_strong <- define_latent_growth_curve_model(df,"STRONG")

ec_cm_strict <- define_latent_growth_curve_model(df, "STRICT")
